...
if (is_readable($filename)) {
	...
	$check_classes = array('WurflCloud_Cache_APC','WurflCloud_Cache_Memcache','WurflCloud_Cache_Memcached',
		'WurflCloud_Cache_File','WurflCloud_Cache_Cookie','WurflCloud_Client_Client','WurflCloud_Client_Config');
...
// Test APC
$apc_class = 'none';
$apc_code_sample = sprintf($code_sample, '$cache = new WurflCloud_Cache_APC();');
if (function_exists('apc_store')) {
	try {
		...
		if ($results['error'] > 0) {
			throw new WurflCloud_Client_Exception("Although APC is not reporting any errors, some of the cache tests failed.");
		}
		$apc_class = 'full';
...
$memcache_class_name = ($memcache)? 'WurflCloud_Cache_Memcache' : 'WurflCloud_Cache_Memcached';
...
if ($memcache || $memcached) {
	try {
		...
		$results = testCache($cache);
		if ($results['error'] > 0) {
			throw new WurflCloud_Client_Exception("Although Memcache is not reporting any errors, some of the cache tests failed.");
		}
		$memcache_class = 'full';
		$cache = null;
...
$sum_results = array(
	array(
		'class_name' => 'WurflCloud_Cache_APC',
		'support' => $apc_class,
	...
	array(
		'class_name' => 'WurflCloud_Cache_File',
		'support' => $file_class,
...
for ($i=0; $i<count($sum_results); $i++) {
	...
	if ($sum_results[$i]['class_name'] == 'WurflCloud_Cache_File' && $sum_results[$i]['support'] == 'partial') {
		$recommended = $sum_results[$i];
		break;
	}
}
